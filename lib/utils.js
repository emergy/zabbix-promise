...
class Client extends Zabbix {
   ...
   *
   * @param {array} items - An array of objects contains the Zabbix host, key
   ...
   * ]})
   */
  static async sender(options, items) {
    try {
      if (!options.server) throw new Error('"server" is missing')

      items.forEach(item => {
        if (!item.host) throw new Error('"host" is missing')
        if (!item.key) throw new Error('"key" is missing')
        if (!item.value) throw new Error('"value" is missing')
      })

      debug('Options received: %o', options)
      debug('Items received: %o', items)
      ...
      */
      ...
      const serverResponse = await new Promise((resolve, reject) => {
        ...
        client.on('end', () => {
          debug('Server response raw: %s', receivedData)
          const jsonData = JSON.parse(receivedData.slice(13))
          debug('Server response parsed JSON: %j', jsonData)
          client.end()

          // Read the Zabbix server response and reject the promise with the
          // error message.
          const errorMessage = `${receivedData}\n` + 'A few things to check,\n' +
